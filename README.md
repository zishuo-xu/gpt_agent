...
`myagent --web` 启动本地 Web 控制台（深色主题 + 品牌绿），主界面为三区布局：

- **左侧任务栏**：项目切换、新建任务、⌘K 搜索，任务按「需要你处理 / 进行中 / 最近任务」分组，行尾状态标记（待处理红点 / 进行中绿圈 / 完成绿勾）；底部进入设置与扩展。
- **中间会话流**：消息气泡、工具调用卡（可展开 diff / 实时输出）、权限审批卡、计划确认弹窗（「需要你确认」角标，批准执行 / 修改计划）；底部输入框支持排队与插队打断。
- **右侧栏**：计划详情（分步进度）、文件改动面板（Edit/Write 路径与 +增/−删 行统计）、消耗与会话信息。移动端右栏折叠为底部抽屉。

首页即新建任务面板：「今天想完成什么？」大输入框（Enter 发送、字数统计、项目选择、先规划开关）、任务选项折叠条（无人值守 / 隔离执行 / 权限档）与示例快捷入口。设置页分「模型 / 通用」两区（供应商与角色模型 / 权限 / 上下文 / 扩展设置），扩展页管理插件启停与调用统计。另有记忆、定时任务、统计面板。
...
- **任务内关键澄清**：Agent 先从仓库和上下文自行判断；只有 API、数据模型、安全边界、不可逆行为或关键 UX 的用户偏好仍会改变实现方向时，才暂停并给出 2–3 个互斥选项。推荐项只是提示，用户必须明确选择或自定义回答；没有超时默认答案。回答后在原会话、原任务中继续，这不是新的自动执行模式
...
- **关键问题仍由人决定**：`/run` 遇到会改变实现方向的关键问题时进入“等待你的回答”，暂停后续工具和机器验收，不会因超时替你选择；回答后沿用同一 taskId 继续。真正需要完全离线跑完的任务，应在启动时把此类偏好写进任务描述或边界
