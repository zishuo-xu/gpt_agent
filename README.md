...
- **定时触发**：`--at 09:00` / `--every 30`（可组合），Web 服务端每 30s 轮询到期自动启动会话；注册 / 取消在「定时任务」面板，持久化于 `scheduled.jsonl`，重启保留
...
- **定时任务面板**：按项目注册 / 取消定时 `/run`（`--at` / `--every`），到期服务端自动启动会话；启动失败自动重试（5 分钟 × 3 次后丢弃），可设**每日花费上限**（超限顺延）
- **任务统计面板**：会话用量聚合（完成 / 失败 / 中断 / tokens / 费用），按天分桶纯 CSS 柱状图 + 会话明细表（含 /run 收尾总结查看）
- **结果触达**：webhook（企业微信 / 飞书 / Bark / 通用 JSON）与 macOS 桌面通知——任务完成 / 失败 / 中断 / 出错 / 审批超时推送，/run 收尾推送含耗时与费用；**保留策略**：超过 `sessionRetentionDays`（默认 30 天）未更新的会话启动与每日自动清理
