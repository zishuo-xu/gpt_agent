...
export function DeliveryWorkbench(props: {
  ...
  return <section className="delivery-workbench delivery-summary" aria-label="交付验收">
    <header className="delivery-hero"><div><p className="delivery-kicker">交付结果</p><h2>{heading}</h2>{d.goal && <p className="delivery-goal">{d.goal}</p>}</div><span className={`delivery-outcome delivery-outcome-${d.outcome}`}>{d.outcome === "completed" ? "结果已生成" : "需要继续处理"}</span></header>
