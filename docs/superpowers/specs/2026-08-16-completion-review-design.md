...
## 产品决策（2026-08-16 用户方向修正）

审查是**开发验证手段，不是产品功能**：运行时默认关闭（`behavior.completionReview` 默认 false），不增加任何成本与延迟；开发/验证 MyAgent 行为时开启或 `/review` 手动触发。用户侧只保留交付结果本身——Agent 干完活直接交付，没有"审查通过/不通过"环节。
...
## 真实回归发现（2026-08-16 实施后）

- 机制全部验证：触发（写操作/任务模式）、打回循环（attempts=2）、上限放行、review_result 事件、CLI/Web 展示（审查卡片 + 头部徽标）、/review 手动触发。打回后主循环模型会主动核验（git log/status）并输出结构化自证——打回机制促使模型自证。
- 同模型审查的质量限制（用户已确认接受）：deepseek-v4-flash 审查时输出"正确"/"审查"等短文本、不遵循三段式格式；结论解析已做宽松识别（含"通过/正确/没问题"且无失败词 → PASS）+ 单字符噪音跳过 + few-shot 示例，但根本改善需更强审查模型（未来可配 models.review）。
- 审查子代理 20 次工具调用/74-91k tokens 属正常（真实核查文件与验证命令）；单次审查成本 ≈ 一次子代理运行。
