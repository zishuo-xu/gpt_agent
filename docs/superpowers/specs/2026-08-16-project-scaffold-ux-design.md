...
- done 判定处（`turn.done` / allTerminated）增加校验：会话存在未完成 todo（非 completed 项）且本轮未调用 TodoWrite → 软拦截：
  - 注入一条系统角色消息："你宣布完成，但任务清单仍有 N 项未完成或未更新。请先更新清单（完成的标 completed，放弃的说明原因），并完成必要验证。若确已完成全部工作，直接再次宣布完成。"
  - 模型继续跑；再次宣布完成则放行。
- 防死循环：同一会话同一任务软拦截最多 2 次，超过则强制 done 并在事件流发 `notify(warn)` 提示"任务宣布完成但 todo 未清空"。
- 不做硬拒绝：无 todo 的会话（闲聊/单步任务）不受影响。
- 触发条件限制：仅当"本轮无 TodoWrite 调用"时拦截，避免模型已经更新过清单但仍有 pending 项（合理，见下）。

补充决策：模型正确更新了 todo（已完成项标 completed、放弃项说明）后仍有 pending 项不算违规——拦截只针对"宣布完成且清单未更新或仍有未完成项"。
