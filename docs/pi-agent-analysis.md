...
| **工具输出截断**（2026-08-03） | 按工具类型差异化上限：Bash 150 行/12KB、Read 256 行/30KB、Grep 200 行/20KB、Glob 300 行/8KB，留头尾掐中段 + 续读提示 |
| **project 级 Key 覆盖提示**（2026-08-03） | 设置页保存全局 Key 时检测项目 local.jsonc 非空 Key，黄色警告 + 输入框内联提示（`/api/config/key-overrides`） |
| **动态工具列表**（2026-08-09） | 按角色动态注入工具集（main 全量 / explore 只读集），system 段落按工具裁剪（原高价值 #3） |
| **会话树分支**（2026-08-09） | fork/switch/回溯 + 分支摘要（`/tree`、`/branch`、Web 分支树；原中价值 #5） |
| **上下文交接**（2026-08-09） | transformMessages + thinking 全链路 + 流式 fallback——跨供应商切换不丢质量（原中价值 #6） |
| **工具结果拆分**（2026-08-09） | summary/output/details 三元组：output 给 LLM、details 进 UI 详情网格（原中价值 #7） |
| **Steer 插队打断**（2026-08-09） | `/steer` 软打断剩余工具调用 + 取消挂起审批（原中价值 #8） |

### 高价值 · 建议优先

| # | 借鉴点 | 对应 Pi 的做法 | 说明 |
|---|---|---|---|
| 1 | ~~工具输出截断~~（已落地 2026-08-03，见"已借鉴"） | 控制单轮注入量 | 差异化上限：Bash 最小 / Read 最大 |
| 2 | **Lazy skills 渐进式披露** | skill 每轮只占一行，用到才加载 | 9 内置 + 插件按角色动态注入；「按需加载」未做——可加载能力，不破坏缓存前缀 |
| 3 | **模型错误信息可操作化** | 透明、可行动 | 把 Insufficient Balance 等翻译成"更新 Key / 配 fallback"的操作指引 |

### 中价值 · 值得研究

> 原 #5-#8（会话树分支 / 上下文交接 / 工具结果拆分 / Steer）已于 2026-08-09 全部落地，见"已借鉴"。

### 低价值 / 需决策

| # | 借鉴点 | 说明 |
|---|---|---|
| 9 | **自修改 + `/reload`** | 热重载已支持（插件面板「重新加载」）；未做的是"agent 改自己代码"的自修改（激进、需护栏，暂不建议） |
